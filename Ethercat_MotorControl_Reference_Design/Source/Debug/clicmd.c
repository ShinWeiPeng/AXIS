...
static int clicmd_EPWMxRegisterDisplay(CONSOLE_Inst *inst, uint8_t index, EPWM_T* EPWMx)
{
    ...
    GPIO_T *GPIOxOffet = 0;
    GPIO_T *GPIOx = (GPIO_T*)GPIOC_BASE;;
	uint16_t cnt[5], cmp_dat[5];
	uint16_t dout;
	...
	for (i=0; i<6; i=i+2)
	{
		memcpy(&cmp_dat[i], (const uint32_t *)&(EPWMx->CMPDAT[i]), sizeof(cmp_dat[i]));
	}
	CONSOLE_PutMessage(inst, "CMPDAT[0]: %d, CMPDAT[2]: %d, CMPDAT[4]: %d\r\n", cmp_dat[0], cmp_dat[2], cmp_dat[4]);
	/*for (i=0; i<3; i++)
	{
	...
	for (i=0; i<6; i=i+2)
	{
	    memcpy(&cnt[i], &(EPWMx->CNT[i]), sizeof(cnt[i]));
	}
	CONSOLE_PutMessage(inst, "CNT[0]: %d, CNT[2]: %d, CNT[4]: %d\r\n", cnt[0], cnt[2], cnt[4]);
...
*/
	memcpy(&dout, (const uint32_t *)&GPIOx->DOUT, sizeof(dout));
    CONSOLE_PutMessage(inst, "PWM_U: %d, PWM_V: %d, PWM_W: %d,\r\n", BIT_TEST(dout, 4), BIT_TEST(dout, 2), BIT_TEST(dout, 0));
	return 0;
}
