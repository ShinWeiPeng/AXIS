...
static int clicmd_GetMotorParaHelp(CONSOLE_Inst *inst);
static int clicmd_GetMotorPara(CONSOLE_Inst *inst, int argc, char **argv);
static int clicmd_GetMotorParaUsage(CONSOLE_Inst *inst);
...
CONSOLE_CmdEntry CLICMD_userCmdTable[]=
{
	...
	{"motor", clicmd_GetMotorPara, NULL, 5},
};
...
static int clicmd_GetMotorPara(CONSOLE_Inst *inst, int argc, char **argv)
{
    ...
    else
    {
        for (i = 1; i < argc; i++)
        {
            if (!strcmp(argv[i], "-svpwm") && strlen(argv[i]) == strlen("-svpwm"))
            {
                CONSOLE_PutMessage(inst, "SVPWM Sampling Time(ms) = %d\r\n", g_MsSvpwmDbgMessage.SamplingTimeMiliSec);

                CONSOLE_PutMessage(inst, "Buf1\r\n");
                for(j = 0; j < MS_SVPWM_DBG_BUF_MAX; j++)
                {
                    CONSOLE_PutMessage(inst, "%i,", g_MsSvpwmDbgMessage.Buf1[j]);
                }
                CONSOLE_PutMessage(inst, "\r\n");

                CONSOLE_PutMessage(inst, "Buf2\r\n");
                for(j = 0; j < MS_SVPWM_DBG_BUF_MAX; j++)
                {
                    CONSOLE_PutMessage(inst, "%i,", g_MsSvpwmDbgMessage.Buf2[j]);
                }
                CONSOLE_PutMessage(inst, "\r\n");

                CONSOLE_PutMessage(inst, "Buf3\r\n");
                for(j = 0; j < MS_SVPWM_DBG_BUF_MAX; j++)
                {
                    CONSOLE_PutMessage(inst, "%i,", g_MsSvpwmDbgMessage.Buf3[j]);
                }
                CONSOLE_PutMessage(inst, "\r\n");

                CONSOLE_PutMessage(inst, "Buf4\r\n");
                for(j = 0; j < MS_SVPWM_DBG_BUF_MAX; j++)
                {
                    CONSOLE_PutMessage(inst, "%d,", g_MsSvpwmDbgMessage.Buf4[j]);
                }
                CONSOLE_PutMessage(inst, "\r\n");
            }
            else if(!strcmp(argv[i], "-record") && strlen(argv[i]) == strlen("-record"))
            {
                i++;
                if (clicmd_DecText2Char((uint8_t*)argv[i], &g_MsSvpwmDbgMessage.record, strlen(argv[i])) != 0)
                {
                    return -1;
                }
            }
            else if(!strcmp(argv[i], "-sampletime") && strlen(argv[i]) == strlen("-sampletime"))
            {
                i++;
                g_MsSvpwmDbgMessage.SamplingTimeMiliSec = 1;
                if (clicmd_DecText2Char((uint8_t*)argv[i], &g_MsSvpwmDbgMessage.SamplingTimeMiliSec, strlen(argv[i])) != 0)
                {
                    return -1;
                }
            }
            else if(!strcmp(argv[i], "-delay") && strlen(argv[i]) == strlen("-delay"))
            {
                i++;
                g_MsSvpwmDbgMessage.DelaySecond = 0;
                if (clicmd_DecText2Char((uint8_t*)argv[i], &g_MsSvpwmDbgMessage.DelaySecond, strlen(argv[i])) != 0)
                {
                    return -1;
                }
            }
...
static int clicmd_GetMotorParaHelp(CONSOLE_Inst *inst)
{
    CONSOLE_PutMessage(inst, "         motor: Process data interface reading\r\n");
    return 1;
} /*
...
{
    clicmd_PdiReadHelp(inst);
    CONSOLE_PutMessage(inst, "Usage: motor -svpwm\r\n");
