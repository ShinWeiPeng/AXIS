...
/*
 * ----------------------------------------------------------------------------
 * Function Name: MS_MapAbzBlkInit()
 * Purpose:
 * Params:
 * Returns:
 * Note:
...
void MS_MapAbzBlkInit(void)
{
    ...
    abzOUT = 0;
    abzPRD = 20;
} /* End of MS_MapAbzBlkInit() */

/*
 * ----------------------------------------------------------------------------
 * Function Name: MS_GetAbzCount()
 * Purpose:
 * Params:
 * Returns:
 * Note:
...
{
	int zPhase_now;
	static int init = 1, zPhase_old = 0;

    if (init)
    {
        init = 0;
        zPhase_old = MH_QEI_GET_CNT_LATCH(pHandle->QeiRegister.pInst);
    }

    iopREL = MH_QEI_GET_CNT(pHandle->QeiRegister.pInst);
    zPhase_now = MH_QEI_GET_CNT_LATCH(pHandle->QeiRegister.pInst);

    if (zPhase_now != zPhase_old)
    {
        /* Detected index(Z) event */
        zPhase_old = zPhase_now;
        iopIDX = zPhase_now;

        if (pHandle->AlignRequest)
        {
            TC_Brake();
            pHandle->AlignRequest = 0;
        }
    }
} /* End of MS_GetAbzCount() */

/* End of incremental_fdbk.c */
