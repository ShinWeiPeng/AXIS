#line 1 "..\\Source\\Mchal\\McHal_eadc.c"
...
#line 33 "..\\Source\\Mchal\\McHal_eadc.c"
...
#line 34 "..\\Source\\Mchal\\McHal_eadc.c"
...
int32_t MH_AdcRegInit(MH_AdcRegConfig_t *pHandle, uint16_t v)
{
	...
	for (i = 0; i < v; i++)
	{
		...
		GPIO_SetMode(pHandle->pPort[i], pHandle->Channel[i], 0x0UL);
		((pHandle ->pPort[i])->DINOFF |= ((pHandle ->Channel[i])<<16));
	}

	 
	EADC_Open(pHandle->pInst, (0UL<<(8)) | (0x3ul << (6)));

	 
	for (i = 0; i < v; i++)
	{
		EADC_ConfigSampleModule(pHandle->pInst, pHandle->SampleModuleMask[i], (0xEUL<<(16)), MH_BitToNum(pHandle->Channel[i]));
	}
...
uint16_t MH_AdcConv(MH_AdcRegConfig_t *pHandle, uint16_t v)
{
	...
	((pHandle ->pInst)->STATUS2 = ((0x1ul << (0))));

	 
	for (i = 0; i < v ; i++)
	{
		pHandle->ConvBuffer[i] = (uint16_t)(((pHandle ->pInst)->DAT[(pHandle ->SampleModuleMask[i])] & (0xfffful << (0))));
	}
...
void MH_AdcStop(MH_AdcRegConfig_t *pHandle, uint16_t v)
{
	...
	for (i = 0; i < v ;i++)
	{
		((pHandle ->pInst)->PENDSTS = (pHandle ->SampleModuleMask[i]));
	}
}  
