#line 1 "..\\Source\\McInterface\\mc_math.c"
...
#line 33 "..\\Source\\McInterface\\mc_math.c"
...
#line 34 "..\\Source\\McInterface\\mc_math.c"
#line 35 "..\\Source\\McInterface\\mc_math.c"
...
void MI_Park(MCMATH_AB_PHASE_T *pPhaseAB)
{
	...
	d_val = ((pPhaseAB->Alpha * (mapMemory[((440)+2)])) + (pPhaseAB->Beta * (mapMemory[((440)+1)]))) >> 8;
	q_val = ((pPhaseAB->Beta * (mapMemory[((440)+2)])) - (pPhaseAB->Alpha * (mapMemory[((440)+1)]))) >> 8;

	 
	d_filtered = d_sum / (mapMemory[((340)+9)]);
	d_sum += (d_val - d_filtered);
	q_filtered = q_sum / (mapMemory[((340)+9)]);
	q_sum += (q_val - q_filtered);

	 
	(mapMemory[((400)+2)]) = (d_filtered * (mapMemory[((460)+7)])) / (mapMemory[((460)+9)]);
	(mapMemory[((400)+3)]) = (q_filtered * (mapMemory[((460)+7)])) / (mapMemory[((460)+9)]);
...
MCMATH_AB_PHASE_T MI_RevPark(void)
{
	...
	if ((mapMemory[((440)+3)]) == SCM_DISABLE)
	{
		return phase_ab;
	}

	if ((mapMemory[((440)+3)]) == SCM_AUTO_CTRL)
	{
		


 
		(mapMemory[((420)+3)]) = (mapMemory[((440)+0)]) + (((mapMemory[((400)+1)]) * (mapMemory[((440)+5)])) / 4096);
	}

	 
	phase_dq.Direct = d_sum / (mapMemory[((420)+13)]);
	d_sum += ((mapMemory[((400)+4)]) - phase_dq.Direct);
	phase_dq.Quadrature = q_sum / (mapMemory[((420)+13)]);
	q_sum += ((mapMemory[((400)+5)]) - phase_dq.Quadrature);

	 
	(mapMemory[((440)+1)]) = MI_GetSIN((mapMemory[((420)+3)]));
	(mapMemory[((440)+2)]) = MI_GetSIN((mapMemory[((420)+3)]) + 90);
	phase_ab.Alpha = ((phase_dq.Direct * (mapMemory[((440)+2)])) - (phase_dq.Quadrature * (mapMemory[((440)+1)]))) >> 8;
	phase_ab.Beta = ((phase_dq.Quadrature * (mapMemory[((440)+2)])) + (phase_dq.Direct * (mapMemory[((440)+1)]))) >> 8;
...
void MI_CalcRotorVector(void)
{
	...
	delta_ang = (mapMemory[((400)+10)]) - (mapMemory[((400)+11)]);
	printf("motor mechanical angle：%d(unit: pulse)\n", delta_ang);

	 
	if (delta_ang > (mapMemory[((480)+4)]))
	{
		(mapMemory[((400)+11)]) += (mapMemory[((480)+4)]);
	}
	...
	printf("angREL(Encoder pulse)：%d(unit: pulse)\n", (mapMemory[((480)+4)]));
	printf("iopIDX(Z phase -> Encoder pulse)：%d(unit: pulse)\n", (mapMemory[((400)+11)]));

	 
	if ((mapMemory[((480)+7)]) & 0x01)
	{
		delta_ang = -delta_ang;
	}
	...
	delta_ang = delta_ang * 360 / (mapMemory[((480)+4)]);
	printf("motor mechanical angle：%d(unit: degree)\n", delta_ang);
	delta_ang = delta_ang * (mapMemory[((480)+5)]);
	printf("motor electrical angle：%d(unit: degree)\n", delta_ang);

	delta_ang -= (mapMemory[((440)+6)]);
	...
	(mapMemory[((440)+0)]) = delta_ang + (((mapMemory[((400)+1)]) * (mapMemory[((440)+4)])) / 4096);
	printf("uvwDEG：%d\n", (mapMemory[((440)+0)]));
	(mapMemory[((440)+1)]) = MI_GetSIN((mapMemory[((440)+0)]));
	(mapMemory[((440)+2)]) = MI_GetSIN((mapMemory[((440)+0)]) + 90);
}  
...
void MI_MapAngBlkInit(void)
{
	(mapMemory[((480)+3)]) = 0x20000;
	(mapMemory[((480)+5)]) = 4;
	(mapMemory[((480)+6)]) = 101;
	(mapMemory[((480)+4)]) = 102400;
	(mapMemory[((480)+8)]) = 400;
	(mapMemory[((480)+9)]) = 512;
	(mapMemory[((480)+7)]) = 0x02;
	(mapMemory[((480)+0)]) = 0;
	(mapMemory[((480)+2)]) = 0;
	(mapMemory[((480)+1)]) = 0;
	(mapMemory[((320)+9)]) = 4;
...
void MI_GetEncAngle_AbzMode(void)
{
	...
	if (rst || (mapMemory[((400)+15)]))
	{
		rst = 0;
		pre_ang = (mapMemory[((400)+10)]);

		if ((mapMemory[((400)+13)]) > 1)
		{
			(mapMemory[((400)+0)]) = 0;
		}
	...
	if ((mapMemory[((480)+7)]) & 0x01)
	{
		(mapMemory[((400)+0)]) -= delta_ang;
	}
	else
	{
		(mapMemory[((400)+0)]) += delta_ang;
	}
	printf("iopANG(cur_ang - pre_ang)：%d(unit: pulse)\n", (mapMemory[((400)+0)]));
}  
...
void MI_CalcEncAngle(uint32_t ResetAngle)
{
	...
	if (!gain || (mapMemory[((400)+15)]) || ResetAngle)
	{
		gain = 0x8000 / (mapMemory[((480)+4)]);
		if (gain < 2)
		{
			gain = 2;
		}
	...
	div_cnt = 0;
    printf("iopREL：%d(unit: pulse)\n", (mapMemory[((400)+10)]));

	 
	cur_ang = (mapMemory[((400)+0)]) * gain;
	printf("cur_ang：%d(iopANG * gain)\n", cur_ang);
	delta_ang = cur_ang - pre_ang;
	printf("delta_ang：%d(cur_ang - pre_ang)\n", delta_ang);
	pre_ang = cur_ang;

	 
	spd = spd_sum / (mapMemory[((320)+9)]);	
	spd_sum += (delta_ang - spd);
	printf("spd_sum：%d\n", spd_sum);

	 
	(mapMemory[((400)+1)]) = spd;
	printf("iopSPD：%d(unit: pulse)\n", (mapMemory[((400)+1)]));
}  
...
void MI_FocCurrController(void)
{
	...
	if ((mapMemory[1009]) > 100)
	{
		MI_CalcEncAngle(0);
	}

	if ((mapMemory[1009]) > 200)
	{
		MI_CalcRotorVector();
	}
	if ((mapMemory[1009]) > 700)
	{
	...
	if ((mapMemory[1009]) > 900)
	{
		if((mapMemory[((240)+0)]) >= LC_AMP_CTRL)
		{
