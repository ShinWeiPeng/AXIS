#line 1 "..\\Source\\Mchal\\McHal_qei.c"
...
#line 33 "..\\Source\\Mchal\\McHal_qei.c"
...
#line 34 "..\\Source\\Mchal\\McHal_qei.c"
...
int32_t MH_QeiRegInit(MH_QeiRegConfig_t *pHandle)
{
	...
	GPIO_SetMode(pHandle->pPortIncEnc, pHandle->PinA | pHandle->PinB | pHandle->PinZ, 0x0UL);
	GPIO_SetSlewCtl(pHandle->pPortIncEnc, pHandle->PinA | pHandle->PinB | pHandle->PinZ, 0x0UL);
	GPIO_SetPullCtl(pHandle->pPortIncEnc, pHandle->PinA | pHandle->PinB | pHandle->PinZ, 0x0UL);
	
	pHandle->pPortIncEnc->SMTEN |= (pHandle->PinA | pHandle->PinB | pHandle->PinZ);

	
    ((pHandle ->pInst)->CTL |= (0x1ul << (25)));
    ((pHandle ->pInst)->CTL |= ((0x1ul << (4)) | (0x1ul << (5)) | (0x1ul << (6))));
    ((pHandle ->pInst)->CTL = ((pHandle ->pInst)->CTL & (~((0x1ul << (3))|(0x7ul << (0))))) | ((0x2<<(0))));
    ((pHandle ->pInst)->CTL = ((pHandle ->pInst)->CTL & (~(0x3ul << (8)))) | ((0x0<<(8))));
    QEI_Start(pHandle->pInst);

	 
	MH_SetMFP(pHandle->pPortWireCtrl, pHandle->PinWireCtrl, 0);
	GPIO_SetMode(pHandle->pPortWireCtrl, pHandle->PinWireCtrl, 0x1UL);
	{if (MC_DISABLE!=0){pHandle ->pPortWireCtrl->DOUT |= pHandle ->PinWireCtrl;}else{pHandle ->pPortWireCtrl->DOUT &= (~pHandle ->PinWireCtrl);}};
