...
#line 33 "..\\Source\\Debug\\ax_uart.c"
...
#line 34 "..\\Source\\Debug\\ax_uart.c"
...
#line 35 "..\\Source\\Debug\\ax_uart.c"
...
#line 36 "..\\Source\\Debug\\ax_uart.c"
...
int AX_UART_Init(void)
{
	...
	UART_Obj.RxBufFree = 512;	
	UART_Obj.TxBufFree = 1024;	
	UART_Obj.Instance = ((UART_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x30000UL));
	
   
	HW_UART_ClkSource(pUartHandle->Instance, (1UL));
	...
	HW_MultiFuncPins(((GPIO_T *) (((uint32_t)0x40000000) + 0x04000UL)), (0x00000080UL)|(0x00000040UL), 7);

	 		
	while ((((pUartHandle ->Instance)->INTSTS & ((0x1ul << (0))))?1:0))
	{
		UART_Read(pUartHandle->Instance, &tmp8, 1);
	}
	UART_EnableInt(pUartHandle->Instance, (0x1ul << (0)));

	
	 
	NVIC_SetPriority(UART0_IRQn, NVIC_EncodePriority(3, 7, 1));
	NVIC_EnableIRQ(UART0_IRQn);

	
	 	
	UART_Open(pUartHandle->Instance, 115200);	
...
int AX_UART_DeInit(void)
{
	...
	UART_DisableInt(UART_Obj.Instance, (0x1ul << (22)) | (0x1ul << (0)));
...
void UART0_IRQHandler(void)
{
	...
	if((Control & (0x1ul << (0))) && (Flags & ((0x1ul << (0)) | (0x1ul << (5)))))	
	{
		 
		while(pUartReg->INTSTS & (0x1ul << (0)))
		{
	...
	if((Control & (0x1ul << (22))) && (Flags & (0x1ul << (22))))
	{
		 		
		while(pUartReg->INTSTS & (0x1ul << (22)))
		{
			 
			pUartReg->DAT = UART_Obj.TxBuf[UART_Obj.TxRdPtr];
			UART_Obj.TxRdPtr++;
			if(UART_Obj.TxRdPtr >= 1024)
			{
				UART_Obj.TxRdPtr = 0;
			}
			UART_Obj.TxBufFree++;
			if (UART_Obj.TxBufFree >= 1024)
			{
				UART_Obj.TxBufFree = 1024;
				((pUartReg)->INTEN &= ~ ((0x1ul << (22))));
				return;
			}
		}
	}
...
char AX_UART_GetChar(void)
{
	...
	if (UART_Obj.RxBufFree != 512)
	{
		((UART_Obj . Instance)->INTEN &= ~ ((0x1ul << (0))));
		ch = UART_Obj.RxBuf[UART_Obj.RxRdPtr];
		UART_Obj.RxBufFree++;
		UART_Obj.RxRdPtr++;
		if(UART_Obj.RxRdPtr >= 512)
		{
			UART_Obj.RxRdPtr = 0;
		}
		((UART_Obj . Instance)->INTEN |= ((0x1ul << (0))));
...
unsigned char AX_UART_PutChar(unsigned char ch)
{
	...
	((UART_Obj . Instance)->INTEN &= ~ ((0x1ul << (22))));
	UART_Obj.TxBuf[UART_Obj.TxWrPtr] = ch;
	UART_Obj.TxBufFree--;
	UART_Obj.TxWrPtr++;
	if (UART_Obj.TxWrPtr >= 1024)
	{
		UART_Obj.TxWrPtr = 0;
	}
	((UART_Obj . Instance)->INTEN |= ((0x1ul << (22))));
