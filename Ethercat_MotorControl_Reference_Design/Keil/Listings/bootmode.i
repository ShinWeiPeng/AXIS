#line 1 "..\\For_SSC_Tool\\Src\\bootmode.c"
...
#line 30 "..\\For_SSC_Tool\\Src\\bootmode.c"
...
#line 31 "..\\For_SSC_Tool\\Src\\bootmode.c"
...
typedef enum{
   ...
   void BL_Stop(void);
    void BL_StartDownload(unsigned long password);
...
#line 33 "..\\For_SSC_Tool\\Src\\bootmode.c"
...
#line 35 "..\\For_SSC_Tool\\Src\\bootmode.c"
...
#line 36 "..\\For_SSC_Tool\\Src\\bootmode.c"





#line 50 "..\\For_SSC_Tool\\Src\\bootmode.c"
...
unsigned short				timerForJumpToLdrom;


unsigned long firmwareAddr = 0x40000+sizeof(UPGRADE_RT_HEAD);
unsigned long parameterAddr_A = 0x38000 + sizeof(UPGRADE_RT_HEAD);
unsigned long parameterAddr_B = 0x78000 + sizeof(UPGRADE_RT_HEAD);

static unsigned long ReadFlashHeader(unsigned long Addr,uint8_t *data);
static unsigned long FirmwareChecksum32(unsigned long Addr,unsigned long length);
...
void BL_StartDownload(unsigned long password0)
{
...
unsigned short BL_Data(unsigned short *pData,unsigned short Size)
{
	...
	int				updateLen = 0;
	unsigned short			writeSize = Size;
	unsigned char	*pSrcData = (unsigned char *)pData;
	static unsigned short parameterAddrChange = 0;
	...
	else if (firmwareObj.State == BL_STATE_START_DOWNLOAD)
	{
		if (firmwareObj.FileOffset == 0)
		{
			...
			firmwareObj.FileLength = ((firmwareHeader . FileLen >> 24 & 0x000000FF) | (firmwareHeader . FileLen >> 8 & 0x0000FF00) | ( firmwareHeader . FileLen << 24 & 0xFF000000) | (firmwareHeader . FileLen << 8 & 0x00FF0000));
			if(firmwareHeader.ImageType == 0)
			{
				if (firmwareObj.FileLength > 0x37FFF)
				{
					SYS_UnlockReg();
					FMC_Close();	 
					SYS_LockReg();
			...
			else if(firmwareHeader.ImageType == 1)
			{
				if (firmwareObj.FileLength > 0x7FFF)
				{
					SYS_UnlockReg();
					FMC_Close();	 
					SYS_LockReg();
	...
	if (firmwareObj.NumberPageErase >  0)
	{
		...
		if(firmwareHeader.ImageType == 0)
		{
			if (FMC_Erase(0x40000+(firmwareObj.PageBeErased*4096)) < 0)
			{
					FMC_Erase(0x40000);
					FMC_Close();                        
					SYS_LockReg();
		...
		else if(firmwareHeader.ImageType == 1)
		{
			...
			uint8_t result = ReadFlashHeader(0x38000,(uint8_t*)&flashHeader32);
			if (result == 1 && FirmwareChecksum32(parameterAddr_A , ((flashHeader32 . FileLen >> 24 & 0x000000FF) | (flashHeader32 . FileLen >> 8 & 0x0000FF00) | ( flashHeader32 . FileLen << 24 & 0xFF000000) | (flashHeader32 . FileLen << 8 & 0x00FF0000))-sizeof(UPGRADE_RT_HEAD))== ~(flashHeader32.Checksum32))
			{
				if (FMC_Erase(0x78000+(firmwareObj.PageBeErased*4096)) < 0)
				{
						FMC_Close();                        
						SYS_LockReg();
						return 0x8003;
			...
			{
				if (FMC_Erase(0x38000+(firmwareObj.PageBeErased*4096)) < 0)
				{
						FMC_Close();                        
						SYS_LockReg();
						return 0x8003;
	...
	while (writeSize > 0)
	{
		...
		if(firmwareHeader.ImageType == 0)
		{
			FMC_Write(0x40000+firmwareObj.FileOffset, dwordData);
			dataRead32 = FMC_Read(0x40000+firmwareObj.FileOffset);
		...
		else if(firmwareHeader.ImageType == 1)
		{
			...
			{
				FMC_Write(0x38000+firmwareObj.FileOffset, dwordData);
				dataRead32 = FMC_Read(0x38000+firmwareObj.FileOffset);
				if (dataRead32 != dwordData)
				{
			...
			{
				FMC_Write(0x78000+firmwareObj.FileOffset, dwordData);
				dataRead32 = FMC_Read(0x78000+firmwareObj.FileOffset);
				if (dataRead32 != dwordData)
				{
	...
	{	
		if(firmwareHeader.ImageType == 0)
		{
			...
			timerForJumpToLdrom = ((unsigned short)((((TIMER_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x11000UL))->CNT & 0xffffful)>>6));
...
{
	unsigned long i;
	unsigned long count = (length >> 2);
	unsigned long cksum = 0, data32;
...
void BL_JumpToLdrom(void)
{
	...
	{
		tmp16 = ((unsigned short)((((TIMER_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x11000UL))->CNT & 0xffffful)>>6));
		if (tmp16 >= timerForJumpToLdrom)
		{
			if ((tmp16 - timerForJumpToLdrom) >= 1000)
			{
				firmwareObj.State = BL_STATE_IDLE;
				SYS_UnlockReg();
				FMC_Open();
