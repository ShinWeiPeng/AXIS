...
int BSP_PDI_EscRead(uint32_t addr, uint8_t *pbuf, uint32_t ByteSize)
{
	...
	HW_EscRead((unsigned long*)pbuf, (unsigned short)addr, (unsigned short)ByteSize);
...
int BSP_PDI_EscWrite(uint32_t addr, uint8_t *pbuf, uint32_t ByteLen)
{
	...
	HW_EscWrite((unsigned long*)pbuf, (unsigned short)addr, (unsigned short)ByteLen);
...
int32_t TEST_PatternGenerator(TEST_PATTERN_TEMP *pPattern, uint8_t *pBuf, uint32_t Len) 
{
	...
	default:
		srand((unsigned int)((unsigned short)((((TIMER_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x11000UL))->CNT & 0xffffful)>>6))+pPattern->Seed);		
		for (i=0; i<Len; i++)
		{
			pBuf[i] = (uint8_t)rand();							
		}					
...
int16_t TEST_PdiMemoryTest(TEST_CONTROL *pCtrl) 
{
		...
		case TEST_ROUND_START:
			...
			srand((unsigned int)((unsigned short)((((TIMER_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x11000UL))->CNT & 0xffffful)>>6)));
		...
		case TEST_RUN:
			...
			pTemp->TxLen = (pTemp->RemainLen > 1024) ? 1024:pTemp->RemainLen;
